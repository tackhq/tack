...
The system SHALL provide a `bolt inventory --list` subcommand (or `bolt inventory list`) that loads the inventory from the specified `-i` source(s) and prints the resolved result as JSON to stdout.

#### Scenario: List static inventory
- **WHEN** user runs `bolt inventory --list -i hosts.yml`
- **THEN** the system SHALL print the parsed inventory as JSON with hosts and groups

#### Scenario: List dynamic inventory
- **WHEN** user runs `bolt inventory --list -i ./ec2-inventory.py`
...
- **WHEN** user runs `bolt inventory --list -i ec2.yml` where the file contains `plugin: ec2`
- **THEN** the system SHALL load via the EC2 plugin and print the resolved inventory as JSON

#### Scenario: No inventory specified
- **WHEN** user runs `bolt inventory --list` without `-i`
...
The JSON output SHALL mirror the Bolt inventory structure with `hosts` and `groups` top-level keys. Host entries SHALL include their vars, SSH config, and any other metadata. Group entries SHALL include their host lists, vars, and connection settings.
...
The system SHALL support `bolt inventory --host <name>` to print details for a single host, including all resolved vars (merged from host-level and group-level).

#### Scenario: Show single host
- **WHEN** user runs `bolt inventory --host web1 -i hosts.yml`
- **THEN** the system SHALL print the host's vars, SSH config, and group memberships as JSON

#### Scenario: Unknown host
- **WHEN** user runs `bolt inventory --host unknown -i hosts.yml` and "unknown" is not in the inventory
