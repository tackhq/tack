...
When `--hosts all` is provided with an inventory file, tack SHALL expand all groups, merge all top-level hosts, deduplicate, and target every resolved host.

#### Scenario: All groups expanded
- **WHEN** user runs `tack run -i inventory.yaml --hosts all playbook.yaml`
...
- **WHEN** user runs `tack run --hosts all playbook.yaml` without `-i`
- **THEN** tack returns error: `--hosts all requires an inventory file (-i flag)`

### Requirement: Distinct error for zero SSM tag matches
When SSM tag resolution succeeds but returns zero instances, tack SHALL produce a specific error message identifying the tags that matched nothing.

#### Scenario: Tags match no running instances
- **WHEN** a group has SSM tags configured and tag resolution returns an empty list
- **THEN** tack returns error mentioning "zero instances" and listing the tags used
