...
The executor SHALL run plays across up to N hosts concurrently when `--forks N` is specified, where N is greater than 1. Each host SHALL execute in its own goroutine with an independent PlayContext and Connector.

#### Scenario: Forks set to 5 with 10 hosts
- **WHEN** `--forks 5` is specified and the play targets 10 hosts
- **THEN** the executor SHALL run up to 5 hosts concurrently, queuing remaining hosts until a slot becomes available

#### Scenario: Default forks value
- **WHEN** `--forks` is not specified
- **THEN** the executor SHALL default to 1 (serial execution), preserving backward compatibility

#### Scenario: Forks greater than host count
- **WHEN** `--forks 20` is specified and the play targets 5 hosts
- **THEN** the executor SHALL run all 5 hosts concurrently without error

### Requirement: Output buffering
Per-host output SHALL be buffered during parallel execution and flushed sequentially in host order after all hosts complete, preventing interleaved output.

#### Scenario: Two hosts complete out of order
- **WHEN** host-b completes before host-a during parallel execution
- **THEN** output SHALL be displayed in order: host-a first, then host-b

#### Scenario: Serial execution output unchanged
- **WHEN** `--forks 1` (default)
- **THEN** output SHALL stream in real-time as before, with no buffering
